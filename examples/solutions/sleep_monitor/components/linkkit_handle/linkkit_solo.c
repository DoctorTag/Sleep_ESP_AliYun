#define PROPERTY_HR "heartRate"
#define PROPERTY_RESP "RespiratoryRate"
#define PROPERTY_SLEEP_STATUS "SleepStatus"
#define PROPERTY_SLEEP_RESULT "sleepResult"
#define PROPERTY_OSAHS_INDICATE "OSAHS_indicate"
#define PROPERTY_SLEEP_SCORE "sleepScore"
...
sleep_monitor_ctx_t g_user_example_ctx = {0};
extern RingbufHandle_t sleep_result_handle;
...
/*
 * Property heartRate paylaod construction and post
 */
static int app_post_heartRate(int hr, sleep_monitor_ctx_t *app_ctx)
{
    int ret = -1;
    char *payload = NULL;
    ...
    cJSON_AddNumberToObject(root, PROPERTY_HR, hr);
    ...
    ret = IOT_Linkkit_Report(app_ctx->master_devid, ITM_MSG_POST_PROPERTY, (unsigned char *)payload, strlen(payload));
    HAL_Free(payload);
    if (ret < 0)
    {
        EXAMPLE_TRACE("app post property \"heartRate\" failed");
        return ret;
    }
    EXAMPLE_TRACE("app post property \"heartRate\" secceed, msgID = %d\r\n", ret);

    return ret;
}

/*
 * Property  paylaod construction and post
 */
static int app_post_property(int prop_val, char *prop, sleep_monitor_ctx_t *app_ctx)
{
    int ret = -1;
    char *payload = NULL;
    cJSON *root = cJSON_CreateObject();
    ...
    cJSON_AddNumberToObject(root, prop, prop_val);
    payload = cJSON_PrintUnformatted(root);
    cJSON_Delete(root);
    ...
    if (ret < 0)
    {
        EXAMPLE_TRACE("app post property  failed");
        return ret;
    }
   // EXAMPLE_TRACE("app post property %s secceed, msgID = %d\r\n", ret, prop);
 ...
 * Property  paylaod construction and post
 */
static int app_post_sleep_result(int item_size, void *result_item, sleep_monitor_ctx_t *app_ctx)
{
    int ret = -1;

    if (item_size > sizeof(inbed_result))
    {
        sleep_result *tmp_result = (sleep_result *)result_item;
        if (tmp_result->result_type == ALG_RESULT_TYPE_COMMON)
        {
            app_post_property(tmp_result->hr, PROPERTY_HR, &g_user_example_ctx);
            app_post_property(tmp_result->resp, PROPERTY_RESP, &g_user_example_ctx);
            app_post_property(tmp_result->status, PROPERTY_SLEEP_STATUS, &g_user_example_ctx);
            app_post_property(tmp_result->OSAHS_ind, PROPERTY_OSAHS_INDICATE, &g_user_example_ctx);
        }
        //Return Item
    }
    else
    {
        inbed_result *pinbed_result = (inbed_result *)result_item;
        if (pinbed_result->result_type == ALG_RESULT_TYPE_INBED)
        {

            app_post_property(pinbed_result->inbed, PROPERTY_SLEEP_STATUS, &g_user_example_ctx);
        }
    }
...
static int linkkit_thread(void *paras)
{
    ...
    memset(&g_user_example_ctx, 0, sizeof(sleep_monitor_ctx_t));
    ...
    while (1)
    {
        ...
        if (sleep_result_handle)
        {
            ptask_result = xRingbufferReceive(sleep_result_handle, &item_size, pdMS_TO_TICKS(5));

            //Check received item
            if (ptask_result != NULL)
            {
                      EXAMPLE_TRACE("Yield xRingbufferReceive len=%d\n",item_size);
  
               app_post_sleep_result(item_size, ptask_result, &g_user_example_ctx);
                //Return Item
                vRingbufferReturnItem(sleep_result_handle, (void *)ptask_result);
            }
        }
    }
